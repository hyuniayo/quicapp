...
repositionAxes( ax ); % *** Yu-Chih: do it by myself (in function)

% If an X-label is present, move it too
repositionXLabel( ax ); % *** Yu-Chih: do it by myself (in function)
...
%-------------------------------------------------------------------------%
    function textLabels = createNewLabels( ax, vals, labels, angle )
        ...
        for ll=1:numel(vals) % *** modified by Yu-Chih
        %for ll=1:numel(labels) % *** modified by Yu-Chih
            textLabels(ll) = text( ...
                'Units', 'Data', ...
        ...
        set(textLabels(end), 'DeleteFcn', @onTextLabelDeleted); % *** modified by Yu-Chih
...
%-------------------------------------------------------------------------%
    function repositionAxes( ax )
        ...
        labelHeight = 0.05; % *** Yu-Chih manully setting for confusion matrix
...
%-------------------------------------------------------------------------%
    function repositionXLabel( ax )
        ...
        removeListeners( ax );
        %labelHeight = getLabelHeight(ax);
        labelHeight = 0.11; % *** Yu-Chih: manually set
