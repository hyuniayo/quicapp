...
% 2015/06/14: change to render chirp for ranging test
%==========================================================================
clear;
close all;
...
if strcmp(PULSE_TYPE,'chirp'),
    ...
    if HAMMING_IS_ENABLED == 1,
        ...
        subplot(2,2,3);
        plot(abs(conv(pulse,pulseNoHamming(end:-1:1)))); legend('correlation w/ hamming');
        
        subplot(2,2,4);
        plot(abs(conv(pulseNoHamming,pulseNoHamming(end:-1:1)))); legend('correlation w/o hamming');
...
fileName = sprintf('source_%drepeat-%dperiod+',REPEAT_CNT,SINGLE_REPEAT_LEN);
if strcmp(PULSE_TYPE,'chirp'),
