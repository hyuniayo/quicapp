function [movingStdAll, movingStdAvg] = AudioParseForIPhoneExternalNoiseTest( TRACE_BASE_FOLDER, payload)
%==========================================================================
...
% 15/11/28: changed from internal noise setting / use more than 4 tags
%         : to finish the test at one time!!
%==========================================================================
close all;
...
if ~exist('TRACE_BASE_FOLDER','var'),
    ...
    %DEFAUGHT_TRACE_BASE_FOLDER = 'Traces/ExternalNoiseFromLaptopTest/DebugOutput/';
    DEFAUGHT_TRACE_BASE_FOLDER = 'Traces/ExternalNoiseFromAnotherUltraphone/DebugOutput/';
...
% do overall moving average
movingStdAvgAll = zeros(length(data) - MOVING_DEPTH, 1);
for i = 1:length(data) - MOVING_DEPTH,
    movingStdAvgAll(i) = std(data(i:i+MOVING_DEPTH)./data(i));
end


dataAll = cell(length(codeStamps)/2,1);
movingStdAll = cell(length(codeStamps)/2,1);
movingStdAvg = zeros(length(codeStamps)/2,1);
for dataIdx = 1:length(dataAll),
    dataAll{dataIdx} = data(dataStamps>codeStamps((dataIdx-1)*2+1)+WAIT_STAMP_OFFSET & dataStamps < codeStamps(dataIdx*2)-WAIT_STAMP_OFFSET); 
    
    movingStdNormal = zeros(length(dataAll{dataIdx}) - MOVING_DEPTH, 1);
    for i = 1:length(dataAll{dataIdx}) - MOVING_DEPTH,
        movingStdNormal(i) = std(dataAll{dataIdx}(i:i+MOVING_DEPTH)./dataAll{dataIdx}(i));
    end
    movingStdAll{dataIdx} = movingStdNormal;
    movingStdAvg(dataIdx) = mean(movingStdAll{dataIdx});
end
