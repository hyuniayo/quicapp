...
% 2015/11/23: calibrate the ext data by start and end instant forces
    ...
    % This is the old setting for FSR 400 (used for Mobisys submission)
    %{
    ...
    % This is the new setting for FlexiForce sensors
    EXT_CALIBRATE_THRE = 1000; %uA
    EXT_CALIBRATE_WIDTH = 100; % this part will be removed from the signals
    EXT_CALIBRATE_TRUNCATE_STAMPE_OFFSET = 10000;
    SMOOTH_LEN = 10;
    ...
    while extOverThreIdx <= length(extOverThre),
        ...
        extCurrentsCorrected(max(startNow-EXT_CALIBRATE_WIDTH,1):min(endNow+EXT_CALIBRATE_WIDTH,length(extCurrentsCorrected))) = 0;
