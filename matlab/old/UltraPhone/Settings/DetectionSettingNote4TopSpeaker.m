...
gds.name = 'DetectionSettingNote4TopSpeaker';

% 3. update audio parse setting
gds.NEED_TO_SEARCH_PILOT = 1;
gds.PILOT_SEARCH_CH_IDXS = [1]; % use top speaker for search
...
gds.SSE_DETECT_CH_IDX = 1; % use the first microphone to read

base = 590;
offset = 20;
gds.SSE_DETECT_SAMPLE_RANGE = base:base+offset; % sample range to get reference
%gds.SSE_DETECT_SAMPLE_RANGE = 595:605;

gds.PSE_DETECT_CH_IDX = 1; % use the first microphone to read
gds.PSE_DETECT_SAMPLE_RANGE = base:base+offset; % sample range to get reference

% 5. squeeze detection setting
gds.SQUEEZE_LEN_TO_CHECK = 30;
gds.USE_TWO_END_CORRECT = 1; % correct data by two end
gds.PEAK_WIN = 8;
gds.PEAK_HARD_THRES_HIGH = 0.3; % this is the hard threshold must feed    
gds.PEAK_HARD_THRES_LOW = 0.2;
